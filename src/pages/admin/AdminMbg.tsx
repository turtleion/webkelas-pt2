...
export default function AdminMbg() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!menu.trim() || !date) { toast.error("Menu dan tanggal wajib diisi."); return; }
    setIsSubmitting(true);
    try {
      if (editingItem) {
        await update(editingItem.id, { day, date, menu, notes: notes || null });
        toast.success("Jadwal MBG berhasil diperbarui.");
      } else {
        await create({ day, date, menu, notes: notes || null });
  ...
  return (
    <AdminLayout>
      ...
      <div className="mt-8">
        <DataTable isLoading={isLoading} error={error} isEmpty={data.length === 0} emptyMessage="Belum ada jadwal MBG.">
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="glass glass-strong max-w-lg border-border/80 text-foreground">
          ...
          <form onSubmit={handleSubmit} className="mt-4 space-y-4">
            ...
            <div><label className="kicker block text-[10px]">Menu</label>
