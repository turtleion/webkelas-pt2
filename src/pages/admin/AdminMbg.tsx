...
export default function AdminMbg() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!menu.trim()) {
      toast.error("Anggota wajib diisi.");
      return;
    }
    ...
    try {
      ...
      toast.error(
        err instanceof Error ? err.message : "Gagal menyimpan jadwal MBG",
      );
    } finally {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.success("Jadwal MBG berhasil dihapus.");
      setDeleteTarget(null);
      await refresh();
    } catch {
      toast.error("Gagal menghapus jadwal MBG");
    } finally {
      setIsDeleting(false);
    }
  };
    ...
    <AdminLayout>
      ...
      <div className="mt-6">
        <DataTable
          ...
          emptyMessage="Belum ada jadwal MBG."
        >
        ...
        <DialogContent className="glass glass-strong max-w-lg border-border/80 text-foreground">
          <DialogHeader>
            <DialogTitle className="font-display text-2xl font-medium tracking-tight">
              {editingItem ? "Ubah Jadwal MBG" : "Tambah Jadwal MBG"}
            </DialogTitle>
          </DialogHeader>
          <form onSubmit={handleSubmit} className="mt-4 space-y-4">
            ...
            <div>
              <label className="kicker block text-[10px]">Anggota</label>
              <Input
                value={menu}
