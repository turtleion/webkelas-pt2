...
> Catatan: proyek ini sudah bermigrasi dari Convex ke Supabase. VLY tetap
> dipakai lewat `src/lib/vly-integrations.ts`; pemakaian dari sisi frontend
> atau fungsi server disesuaikan kebutuhan (contoh Convex di bawah hanya
> referensi lama).
...
3. The integration key (`VLY_INTEGRATION_KEY`) must never be exposed to the client
4. (Dulu Convex action `"use node"`; kini tinggal dipanggil dari kode server/fungsi bila perlu)
